...
//        FirebaseRecyclerOptions reviewsOptions = new FirebaseRecyclerOptions.Builder<Review>().setQuery(reviewsQuery, Review.class).build();
        ...
        FirebaseRecyclerAdapter<Review,ViewReviewRegisterUser.ReviewViewHolderR> FB_recAd = new FirebaseRecyclerAdapter<Review, ViewReviewRegisterUser.ReviewViewHolderR>(
            ...
            @Override
            public void populateViewHolder( ViewReviewRegisterUser.ReviewViewHolderR holder, Review model, int position) {
//                return null;
//                Log.d("error","error"+" 2");
                                ...
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    ...
                                    if(dataSnapshot.hasChild("likes")){
                                        if (dataSnapshot.child("likes").hasChild(user_key)) {
                                            ...
                                            LikeDatabase.child("likes").child(user_key).removeValue();
                                            int newNumLike = Integer.parseInt(dataSnapshot.child("numLike").getValue().toString());
                                            newNumLike=(newNumLike-1);
                                            LikeDatabase.child("numLike").setValue(Integer.toString(newNumLike));

                                            }
                                            else if(dataSnapshot.child("likes").child(user_key).getValue().equals("dislike")){
                                                LikeDatabase.child("likes").child(user_key).setValue("like");

                                                int newNumDisLike = Integer.parseInt(dataSnapshot.child("numDisLike").getValue().toString());
                                                newNumDisLike=(newNumDisLike-1);
                                                LikeDatabase.child("numDisLike").setValue(Integer.toString(newNumDisLike));
                                ...
                                if(prodisLike) {
                                    ...
                                    if (dataSnapshot.child("likes").hasChild(user_key)) {
                                        if(dataSnapshot.child("likes").child(user_key).getValue().equals("dislike")){
                                            disLikeDatabase.child("likes").child(user_key).removeValue();
                                            int newNumDisLike = Integer.parseInt(dataSnapshot.child("numDisLike").getValue().toString());
                                            newNumDisLike=(newNumDisLike-1);
                                            disLikeDatabase.child("numDisLike").setValue(Integer.toString(newNumDisLike));

                                        }
                                        else if(dataSnapshot.child("likes").child(user_key).getValue().equals("like")){
                                            disLikeDatabase.child("likes").child(user_key).setValue("dislike");

                                            int newNumLike = Integer.parseInt(dataSnapshot.child("numLike").getValue().toString());
                                            newNumLike=(newNumLike-1);
                                            disLikeDatabase.child("numLike").setValue(Integer.toString(newNumLike));
...
//        Log.d("error","error"+" 5");
    ...
    public static class ReviewViewHolderR extends RecyclerView.ViewHolder{
